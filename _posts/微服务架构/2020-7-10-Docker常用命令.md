...
## 使用私有仓库管理镜像
使用Docker Registry2.0搭建私有仓库：`docker run -d -p 5000:5000 --restart=always --name registry2 registry:2`
将镜像推送到私有仓库：
- 修改镜像标签：`docker tag sunmood/microservice-discovery-eureka:1.0 localhost:5000/sunmood/microservice-discovery-eureka:1.0`
